...
#         'Chose feature detection algorithm',
#         ('ORB', 'SIFT', 'SURF'))
# st.write('You selected:', feature_detector)

# '''Params of rendering and perfomance'''
st.title("Configure params for worzkone")
st.sidebar.markdown("Params of rendering")
frameST = st.empty() 

draw_matches = st.sidebar.checkbox("Chose to draw matches or not",
                                   value=False)
frame_width = st.sidebar.number_input("Width of input frame",
                                      min_value=120, max_value=1920,
                                      value=640)
frame_height = int(3/4 * frame_width)
focal_lenght = st.sidebar.number_input("Focal lenght of camera",
                                        min_value=0.0, max_value=2000.0,
                                        value=800.0)
marker_size = st.sidebar.number_input("Size of marker",
                                       min_value=16, max_value=frame_height,
                                       value=200)
path_to_marker = st.sidebar.text_input("Write path to custom marker in .jpg or .png formats",
                                       "./external/test_column.jpg")
full_path_to_marker = os.path.abspath(path_to_marker)
path_to_obj = st.sidebar.text_input("Write path to custom area form",
                                    "./external/Cylinder.obj")
full_path_to_obj = os.path.abspath(path_to_obj)
scale_factor_model = st.sidebar.number_input("Write scaled factor for 3d model of workzone",
                                             1e-4, 1e4, (100.0))

# '''Params of feature detector and descriptor'''
st.sidebar.markdown("Params of detector")
scale_factor_feat_det = st.sidebar.slider('Chose scale factor for feature detector',
                                           1.0, 3.0, (1.1))

max_num_of_features = st.sidebar.slider("Select number of features",
                                        30, 5000, (1000)
)

num_of_levels = st.sidebar.number_input("Write number of level for feature detector",
...
if st.sidebar.button('Save and Exit'):
